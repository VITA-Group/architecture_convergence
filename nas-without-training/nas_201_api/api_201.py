...
class NASBench201API(NASBenchMetaAPI):
  ...
  def reload(self, archive_root: Text = None, index: int = None):
    ...
    for idx in indexes:
      ...
      # assert isinstance(xdata, dict) and 'full' in xdata and 'less' in xdata, 'invalid format of data in {:}'.format(xfile_path) # TODO
      assert isinstance(xdata, dict) and 'full' in xdata, 'invalid format of data in {:}'.format(xfile_path)
      hp2archres = OrderedDict()
      # hp2archres['12'] = ArchResults.create_from_state_dict(xdata['less']) # TODO
